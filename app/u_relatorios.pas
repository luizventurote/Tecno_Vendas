...
begin
 ...
 // nomerep := ('RelatoriosUsuario')+ '\' + Relats_UsurArquivo.AsString;
  nomerep := (ExtractFilePath(Application.ExeName) + '\Relatorios\RelatoriosUsuario')+ '\' + Relats_UsurArquivo.AsString;
...
begin
  ...
  case RelatsAtual of
      1: nomerep := (ExtractFilePath(Application.ExeName) + '\Relatorios\RelatoriosSistema')+ '\' + Relats_UsurArquivo.AsString;
      2: nomerep := (ExtractFilePath(Application.ExeName) + '\Relatorios\RelatoriosUsuario')+ '\' + Relats_UsurArquivo.AsString;
      end;
      ppreport1.template.FileName := nomerep;
      ppreport1.template.LoadFromFile;
  end;
 ...
 // nomerep := ('Relatorios\RelatoriosUsuario')+ '\' + Relats_UsurArquivo.AsString;
  ppReport1.template.FileName    := (ExtractFilePath(Application.ExeName) + '\Relatorios\RelatoriosUsuario')+ '\' + _tela + ' '+ inttostr(seq) + ' - '+nomerep+'.rtm';
...
begin
  ...
  begin
      ...
      nomearq        := (ExtractFilePath(Application.ExeName) + '\Relatorios\RelatoriosUsuario')+ '\' + Relats_UsurArquivo.AsString;
      deletefile(nomearq);
      Relats_Usur.delete;
  end;
...
//      dm_dados.mConfig.Open;
  ...
  if Ds.Active then Ds.EmptyDataSet
     ...
     // 1: nomerep := ('Relatorios\RelatoriosSistema')+ '\' + Relats_UsurArquivo.AsString;

   //  /*Mudei aqui*/

  //Dir := f_principal.BuscaParamTexto('RelatoriosSistema');
     Dir :=('Relatorios\RelatoriosSistema');
  //if Origem = 2 then Dir := f_principal.BuscaParamTexto('RelatoriosUsuario');
  if Origem = 2 then Dir := (ExtractFilePath(Application.ExeName) + '\Relatorios\RelatoriosUsuario');
...
begin
    ...
    begin
        ...
        nomerep := (ExtractFilePath(Application.ExeName) + '\Relatorios\RelatoriosSistema')+ '\' + Relats_UsurArquivo.AsString;
    end;
    2:
    begin
